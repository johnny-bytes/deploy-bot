...
export const MainLayout: React.FC = () => {
  ...
  const onApplicationSelected = (item: PivotItem) => {
    setSelectedApplication(data.find((app) => app.id === item.props.itemKey))
  }
  ...
  return (
    <React.Fragment>
      <Stack className={mainStyles.navigation} horizontal verticalAlign="center" tokens={{ childrenGap: '1rem', padding: '0 1rem' }}>
        ...
        <Stack.Item>
          <Pivot styles={pivotStyle} selectedKey={selectedApplication?.id} onLinkClick={onApplicationSelected}>
            {data.map((app) => (
              <PivotItem
                key={app.id}
                itemKey={app.id}
